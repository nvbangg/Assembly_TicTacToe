...
.data               
    ...
    pos_x db 1, 1, 1, 5, 5, 5, 9, 9, 9   ; tọa độ x của mỗi ô
    pos_y db 1, 3, 5, 1, 3, 5, 1, 3, 5   ; tọa độ y của mỗi ô
    ...
    khung_bang db 13,10, '   |   |   ',13,10, '-----------',13,10, '   |   |   ',13,10, '-----------',13,10, '   |   |   ',13,10,'$' ; khung bảng không có X/O
...
ve_bang proc    
    ...
    ; In khung bảng cố định
    mov ah, 9           
    lea dx, khung_bang      ; in khung bảng không có X/O
    int 21h
    
    ; Vẽ từng ký tự với màu sắc tương ứng
    mov cx, 9               ; cx=9 để lặp 9 lần
    mov si, 0               ; đặt si=0 (bắt đầu từ ô đầu tiên)
    
    draw_cells:             ; vẽ từng ô với màu sắc
        mov ah, 02h         ; hàm 02h của ngắt 10h để đặt vị trí con trỏ
        mov bh, 0           ; trang hiển thị 0
        mov dh, pos_y[si]   ; tọa độ y
        mov dl, pos_x[si]   ; tọa độ x
        int 10h             ; đặt vị trí con trỏ
        
        mov ah, 09h         ; hàm 09h của ngắt 10h để hiển thị ký tự có màu
        mov al, ki_tu[si]   ; lấy ký tự cần hiển thị
        
        ; Xác định màu cho ký tự
        cmp al, 'X'         ; nếu là X, màu đỏ
        je x_color
        cmp al, 'O'         ; nếu là O, màu xanh lá
        je o_color
        mov bl, 0Fh         ; nếu là số, màu trắng
        jmp display_char
        
    x_color:
        mov bl, 0Ch         ; màu đỏ cho X (0C = đỏ)
        jmp display_char
        
    o_color:
        mov bl, 0Ah         ; màu xanh lá cho O (0A = xanh lá)
        
    display_char:
        mov cx, 1           ; hiển thị 1 ký tự
        int 10h             ; gọi ngắt để hiển thị
        
        inc si              ; tăng si (chuyển đến ô tiếp theo)
        cmp si, 9           ; kiểm tra đã vẽ đủ 9 ô chưa
        jl draw_cells       ; nếu chưa, tiếp tục vẽ
